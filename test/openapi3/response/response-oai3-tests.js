...
describe("oai3 - response tests", function () {
  ...
  describe("check headers", function () {
    ...
    it("invalid type for headers", function () {
      let isMatch = schemaEndpoint.validate({
        headers: { "x-zooz-request-id": "123" },
      });
      expect(schemaEndpoint.errors).to.be.eql([
        {
          keyword: "format",
          instancePath: "/headers/x-zooz-request-id",
  ...
  describe("check body", function () {
    describe("simple body", function () {
      ...
      it("valid body - quantitive test", function () {
        let schemaEndpoint =
          schema["/many-body-fields"]["post"].responses["201"];
        let isMatch = schemaEndpoint.validate({
          body: {
            fieldNum1: 1,
            fieldNum2: 2,
    ...
    describe("schema with route", function () {
      ...
      it("missing required field", function () {
        ...
        expect(schemaEndpoint.errors[0].message).to.be.equal(
          "must have required property 'bark'"
        );
        expect(schemaEndpoint.errors[1].message).to.be.equal(
          'must match pattern "^\\d+$"'
        );
        expect(schemaEndpoint.errors[2].message).to.be.equal(
    ...
    describe("body with discriminator", function () {
      ...
      describe("discriminator-pet", function () {
        ...
        before(function () {
          schemaEndpoint =
            schema["/pet-discriminator"]["post"].responses["201"];
        });
        it("missing discriminator field", function () {
          ...
          expect(schemaEndpoint.errors[0].instancePath).to.equal("/body/type");
      ...
      describe("discriminator-multiple pet", function () {
        before(function () {
          schemaEndpoint =
            schema["/pet-discriminator-multiple"]["post"].responses["201"];
        });
        it("missing discriminator field", function () {
          let isMatch = schemaEndpoint.validate({
            body: {
        ...
        it("missing discriminator field on the on inside discriminator", function () {
          ...
          expect(schemaEndpoint.errors[0].instancePath).to.equal("/body/model");
          expect(schemaEndpoint.errors[0].keyword).to.equal("enum");
          expect(schemaEndpoint.errors[0].params.allowedValues).to.eql([
            "small_dog",
            "big_dog",
        ...
        it("when discriminator type is dog_multiple and model small_dog and missing root field name and specific plane field", function () {
          ...
          expect(schemaEndpoint.errors[0].message).to.equal(
            "must have required property 'max_length'"
          );
          expect(schemaEndpoint.errors[1].message).to.equal(
            "must have required property 'name'"
          );
          expect(schemaEndpoint.errors[2].message).to.equal(
            "must have required property 'dog_age'"
          );
          expect(isMatch).to.be.false;
        });
      ...
      describe("discriminator-mapping pet", function () {
        before(function () {
          schemaEndpoint =
            schema["/pet-discriminator-mapping"]["post"].responses["201"];
        });
        ...
        it("when discriminator type is mapped_dog and model small_dog and missing root field name and specific dog field", function () {
          ...
          expect(schemaEndpoint.errors[1].instancePath).to.equal("/body");
          expect(schemaEndpoint.errors[1].keyword).to.equal("required");

          expect(schemaEndpoint.errors[2].message).to.equal(
            "must have required property 'dog_age'"
          );
          expect(schemaEndpoint.errors[2].instancePath).to.equal("/body");
  ...
  describe("check body and headers", () => {
    ...
    it("valid headers and bad body validation", function () {
      ...
      expect(schemaEndpoint.errors[0].instancePath).to.be.equal("/body");
      expect(schemaEndpoint.errors[0].message).to.be.equal(
        "must have required property 'bark'"
      );
      expect(schemaEndpoint.errors[1].instancePath).to.be.equal("/body/fur");
      expect(schemaEndpoint.errors[1].message).to.be.equal("must be string");
      expect(schemaEndpoint.errors[2].instancePath).to.be.equal("/body");
      expect(schemaEndpoint.errors[2].message).to.be.equal(
        "must match exactly one schema in oneOf"
      );
    ...
    it("bad headers and body validation", function () {
      ...
      expect(schemaEndpoint.errors[3].instancePath).to.be.equal(
        "/headers/x-zooz-request-id"
      );
      expect(schemaEndpoint.errors[3].message).to.be.equal(
