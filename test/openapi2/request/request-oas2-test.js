...
describe("oai2 - request tests", () => {
  describe("init function tests", function () {
    it("must reject the promise in case the file doesn't exists", function () {
      const swaggerPath = path.join(
        __dirname,
        "./yaml/pet-store-swagger1.yaml"
      );
      return expect(() => {
    ...
    it("must resolve without formats", function () {
  ...
  describe("Simple server - no options", function () {
    ...
    it("bad header - invalid pattern", function () {
      ...
      expect(paramsValidationErrors).to.eql([
        {
          keyword: "pattern",
          instancePath: "/headers/api-version",
    ...
    it("bad header - empty header", function () {
      ...
      expect(paramsValidationErrors).to.eql([
        {
          keyword: "minLength",
          instancePath: "/headers/request-id",
    ...
    it("bad body - wrong format in array item body (second item)", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: [
          { name: "name", tag: "tag", test: { field1: "enum1" } },
          {
            name: "name",
            tag: "tag",
            test: { field1: 1234 },
  ...
  describe("Simple server - type coercion enabled", function () {
    ...
    it("request with wrong parameter type - should keep null values as null when (invalid) swagger with multiple types is provided", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: [
          { name: 1, tag: "tag", test: { field1: "enum1", field3: null } },
        ],
        path: "/pets",
        method: "put",
      });
  ...
  describe("Simple server - with base path", function () {
    ...
    before(function () {
      const swaggerPath = path.join(
        __dirname,
        "./yaml/pet-store-swagger-with-base-path.yaml"
      );
      schemas = schemaValidatorGenerator.buildSchemaSync(swaggerPath, options);
    });
    it("valid request - should pass validation", function () {
    ...
    it("bad request - wrong content-type (should be application/json)", function () {
      ...
      expect(paramsValidationErrors[0].errors.message).to.equal(
        "content-type must be one of application/json"
      );

      expect(paramsValidationErrors[0].errors.params["content-type"]).to.eql(
  ...
  describe("Server with options", function () {
    ...
    it("bad request - wrong content-type (should be application/json)", function () {
      ...
      expect(paramsValidationErrors[0].errors.message).to.equal(
        "content-type must be one of application/json,form-data"
      );
  ...
  describe("Formats", function () {
    let schemas;
    let options = {
      formats: [{ name: "abcName", pattern: /abc/ }],
      contentTypeValidation: true,
    };
    before(function () {
      const swaggerPath = path.join(
        __dirname,
        "./yaml/pet-store-swagger-formats.yaml"
      );
      schemas = schemaValidatorGenerator.buildSchemaSync(swaggerPath, options);
    });
  ...
  describe("Inheritance", function () {
    ...
    before(function () {
      const swaggerPath = path.join(
        __dirname,
        "./yaml/pet-store-swagger-inheritance.yaml"
      );
      schemas = schemaValidatorGenerator.buildSchemaSync(swaggerPath, options);
    });
    ...
    it("must fail for wrong value in discriminator", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: {
          petType: "dog",
      ...
      expect(bodyValidationErrors[0].instancePath).to.equal("/petType");
      expect(bodyValidationErrors[0].params.allowedValues).to.eql([
        "Cat",
        "Dog",
      ]);
    });
    it("must fail for missing discriminator key", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: { name: "name", tag: "tag", test: { field1: "1234" } },
        path: "/pets",
      ...
      expect(bodyValidationErrors[0].instancePath).to.equal("/petType");
      expect(bodyValidationErrors[0].message).to.equal(
    ...
    it("must fail for missing attribute in inherited object (Dog)", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: {
          petType: "Dog",
    ...
    it("must fail for missing attribute in inherited object (cat)", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: {
          petType: "Cat",
    ...
    it("must fail for missing attribute in inherited object (parent)", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
  ...
  describe("FormData", function () {
    ...
    it("missing required file should fail", function () {
      ...
      expect(paramsValidationErrors[0].errors.message).to.equal(
        "Missing required files: sourceFile"
      );
      expect(paramsValidationErrors[0].errors.params.requiredFiles).to.eql([
        "sourceFile",
      ]);
      expect(paramsValidationErrors[0].errors.params.missingFiles).to.eql([
        "sourceFile",
      ]);
      expect(paramsValidationErrors[0].instancePath).to.equal("/files");
    });
    it("extra files exists but not allowed should fail", function () {
      ...
      expect(paramsValidationErrors[0].errors.message).to.equal(
        "Extra files are not allowed. Not allowed files: sourceFile1"
      );
      expect(paramsValidationErrors[0].errors.params.allowedFiles).to.eql([
        "sourceFile",
        "optionalFile",
      ]);
      expect(paramsValidationErrors[0].errors.params.extraFiles).to.eql([
        "sourceFile1",
      ]);
      expect(paramsValidationErrors[0].instancePath).to.equal("/files");
    });
  ...
  describe("Keywords", function () {
    ...
    let schemas,
      options = {
        keywords: [range, prohibited],
        expectFormFieldsInBody: true,
      };
    before(function () {
      const swaggerPath = path.join(
        __dirname,
        "./yaml/custom-keywords-swagger.yaml"
      );
      schemas = schemaValidatorGenerator.buildSchemaSync(swaggerPath, options);
    });
    ...
    it("must be failed by the range keyword", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: { age: 50 },
        path: "/keywords",
    ...
    it("must be failed by the prohibited keyword", function () {
      let bodyValidationErrors = validateBody({
        schemas: schemas,
        body: { ages: 20, age: 20 },
        path: "/keywords",
